...
// StoreCode implements WasmerEngine.
func (t *TrackingWasmerEngine) StoreCode(code wasmvm.WasmCode, gasLimit uint64) (wasmvm.Checksum, uint64, error) {
	return t.vm.StoreCode(code, gasLimit)
}

// StoreCodeUnchecked implements WasmerEngine.
func (t *TrackingWasmerEngine) StoreCodeUnchecked(code wasmvm.WasmCode) (wasmvmtypes.Checksum, error) {
	return t.vm.StoreCodeUnchecked(code)
}

func (t *TrackingWasmerEngine) SimulateStoreCode(code wasmvm.WasmCode, gasLimit uint64) (wasmvm.Checksum, uint64, error) {
	return t.vm.SimulateStoreCode(code, gasLimit)
}
...
func (t *TrackingWasmerEngine) MigrateWithInfo(ctx sdk.Context, checksum wasmvm.Checksum, env wasmvmtypes.Env, migrateMsg []byte, migrateInfo wasmvmtypes.MigrateInfo, store PrefixStoreInfo, goapi wasmvm.GoAPI, querier QuerierWithCtx, gasMeter wasmvm.GasMeter, gasLimit uint64, deserCost wasmvmtypes.UFraction) (*wasmvmtypes.ContractResult, uint64, error) {
	const CurrentOperation = ContractOperationMigrateWithInfo
	...
	prefixStore := prefix.NewStore(gaskv.NewStore(store.Store, &contractMeter, stypes.KVGasConfig()), store.PrefixKey)
	response, vmGasUsed, err := t.vm.MigrateWithInfo(checksum, env, migrateMsg, migrateInfo, NewStoreAdapter(prefixStore), goapi, querier, gasMeter, gasLimit, deserCost)
...
func (t *TrackingWasmerEngine) GetPinnedMetrics() (*wasmvmtypes.PinnedMetrics, error) {
	return t.vm.GetPinnedMetrics()
}
...
func (t *TrackingWasmerEngine) IBCSourceCallback(ctx sdk.Context, codeID wasmvm.Checksum, env wasmvmtypes.Env, msg wasmvmtypes.IBCSourceCallbackMsg, store PrefixStoreInfo, goapi wasmvm.GoAPI, querier QuerierWithCtx, gasMeter wasmvm.GasMeter, gasLimit uint64, deserCost wasmvmtypes.UFraction) (*wasmvmtypes.IBCBasicResult, uint64, error) {
	const CurrentOperation = ContractOperationIBCSourceCallback
	var contractAddress = env.Contract.Address
	...
	response, vmGasUsed, err := t.vm.IBCSourceCallback(codeID, env, msg, NewStoreAdapter(prefixStore), goapi, querier, gasMeter, gasLimit, deserCost)
...
func (t *TrackingWasmerEngine) IBCDestinationCallback(ctx sdk.Context, codeID wasmvm.Checksum, env wasmvmtypes.Env, msg wasmvmtypes.IBCDestinationCallbackMsg, store PrefixStoreInfo, goapi wasmvm.GoAPI, querier QuerierWithCtx, gasMeter wasmvm.GasMeter, gasLimit uint64, deserCost wasmvmtypes.UFraction) (*wasmvmtypes.IBCBasicResult, uint64, error) {
	const CurrentOperation = ContractOperationIBCDestinationCallback
	var contractAddress = env.Contract.Address
	...
	response, vmGasUsed, err := t.vm.IBCDestinationCallback(codeID, env, msg, NewStoreAdapter(prefixStore), goapi, querier, gasMeter, gasLimit, deserCost)
