...
func (t *testQuerier) Query(request wasmvmtypes.QueryRequest, gasLimit uint64) ([]byte, error) {
	...
	response, gasUsed, err := t.Vm.Query(
		...
		[]byte{},
		PrefixStoreInfo{Store: store.NewCommitMultiStore(db.NewMemDB(), log.NewNopLogger(), storemetrics.NewNoOpMetrics()).GetCommitKVStore(storetypes.NewKVStoreKey("test")), PrefixKey: []byte{0x1}},
		cosmwasm.GoAPI{},
		t,
...
func (l *loggingVM) StoreCode(code cosmwasm.WasmCode, gasLimit uint64) (wasmvm.Checksum, uint64, error) {
	if l.Fail {
	...
	return cosmwasm.Checksum{}, 1, nil
}
...
func (l *loggingVM) Execute(code cosmwasm.Checksum, env wasmvmtypes.Env, info wasmvmtypes.MessageInfo, executeMsg []byte, store cosmwasm.KVStore, goapi cosmwasm.GoAPI, querier cosmwasm.Querier, gasMeter cosmwasm.GasMeter, gasLimit uint64, deserCost wasmvmtypes.UFraction) (*wasmvmtypes.ContractResult, uint64, error) {
	if l.Fail {
		return &wasmvmtypes.ContractResult{}, 0, errTestFail
	}
	if l.ShouldEmulateQuery && len(l.QueryContracts) > 0 {
...
func (l *loggingVM) Query(code cosmwasm.Checksum, env wasmvmtypes.Env, queryMsg []byte, store cosmwasm.KVStore, goapi cosmwasm.GoAPI, querier cosmwasm.Querier, gasMeter cosmwasm.GasMeter, gasLimit uint64, deserCost wasmvmtypes.UFraction) (*wasmvmtypes.QueryResult, uint64, error) {
	if l.Fail {
		return &wasmvmtypes.QueryResult{}, 0, errTestFail
	}
	...
	return &wasmvmtypes.QueryResult{Ok: []byte{1}}, currentOperationGas, nil
}

func (l *loggingVM) Migrate(checksum cosmwasm.Checksum, env wasmvmtypes.Env, migrateMsg []byte, store cosmwasm.KVStore, goapi cosmwasm.GoAPI, querier cosmwasm.Querier, gasMeter cosmwasm.GasMeter, gasLimit uint64, deserCost wasmvmtypes.UFraction) (*wasmvmtypes.ContractResult, uint64, error) {
	if l.Fail {
		return &wasmvmtypes.ContractResult{}, 0, errTestFail
	}
	if l.ShouldEmulateQuery && len(l.QueryContracts) > 0 {
...
func (l *loggingVM) Sudo(checksum cosmwasm.Checksum, env wasmvmtypes.Env, sudoMsg []byte, store cosmwasm.KVStore, goapi cosmwasm.GoAPI, querier cosmwasm.Querier, gasMeter cosmwasm.GasMeter, gasLimit uint64, deserCost wasmvmtypes.UFraction) (*wasmvmtypes.ContractResult, uint64, error) {
	if l.Fail {
		return &wasmvmtypes.ContractResult{}, 0, errTestFail
	}
	if l.ShouldEmulateQuery && len(l.QueryContracts) > 0 {
...
func (l *loggingVM) Reply(checksum cosmwasm.Checksum, env wasmvmtypes.Env, reply wasmvmtypes.Reply, store cosmwasm.KVStore, goapi cosmwasm.GoAPI, querier cosmwasm.Querier, gasMeter cosmwasm.GasMeter, gasLimit uint64, deserCost wasmvmtypes.UFraction) (*wasmvmtypes.ContractResult, uint64, error) {
	if l.Fail {
		return &wasmvmtypes.ContractResult{}, 0, errTestFail
	}
	if l.ShouldEmulateQuery && len(l.QueryContracts) > 0 {
...
func (l *loggingVM) IBCChannelOpen(checksum cosmwasm.Checksum, env wasmvmtypes.Env, channel wasmvmtypes.IBCChannelOpenMsg, store cosmwasm.KVStore, goapi cosmwasm.GoAPI, querier cosmwasm.Querier, gasMeter cosmwasm.GasMeter, gasLimit uint64, deserCost wasmvmtypes.UFraction) (*wasmvmtypes.IBCChannelOpenResult, uint64, error) {
	...
	return &wasmvmtypes.IBCChannelOpenResult{}, currentOperationGas, nil
}
...
func (l *loggingVM) IBCChannelConnect(checksum cosmwasm.Checksum, env wasmvmtypes.Env, channel wasmvmtypes.IBCChannelConnectMsg, store cosmwasm.KVStore, goapi cosmwasm.GoAPI, querier cosmwasm.Querier, gasMeter cosmwasm.GasMeter, gasLimit uint64, deserCost wasmvmtypes.UFraction) (*wasmvmtypes.IBCBasicResult, uint64, error) {
	if l.Fail {
		return &wasmvmtypes.IBCBasicResult{}, 0, errTestFail
	}
	if l.ShouldEmulateQuery && len(l.QueryContracts) > 0 {
		querier.Query(wasmvmtypes.QueryRequest{Wasm: &wasmvmtypes.WasmQuery{Raw: &wasmvmtypes.RawQuery{ContractAddr: l.QueryContracts[0]}}}, uint64(len(l.QueryContracts))-1)
	}
	...
	return &wasmvmtypes.IBCBasicResult{}, currentOperationGas, nil
}
...
func (l *loggingVM) IBCPacketAck(checksum cosmwasm.Checksum, env wasmvmtypes.Env, ack wasmvmtypes.IBCPacketAckMsg, store cosmwasm.KVStore, goapi cosmwasm.GoAPI, querier cosmwasm.Querier, gasMeter cosmwasm.GasMeter, gasLimit uint64, deserCost wasmvmtypes.UFraction) (*wasmvmtypes.IBCBasicResult, uint64, error) {
	if l.Fail {
		return &wasmvmtypes.IBCBasicResult{}, 0, errTestFail
	}
	if l.ShouldEmulateQuery && len(l.QueryContracts) > 0 {
...
func TestGasTrackingVMInstantiateAndQuery(t *testing.T) {
	cms := store.NewCommitMultiStore(db.NewMemDB(), log.NewTestLogger(t), storemetrics.NewNoOpMetrics())
	...
	_, gasUsed, err := gasTrackingVm.Instantiate(
		...
		[]byte{},
		PrefixStoreInfo{Store: store.NewCommitMultiStore(db.NewMemDB(), log.NewTestLogger(t), storemetrics.NewNoOpMetrics()).GetCommitKVStore(storetypes.NewKVStoreKey("test")), PrefixKey: []byte{0x1}},
		cosmwasm.GoAPI{},
		&testQuerier,
		storetypes.NewInfiniteGasMeter(),
		50,
