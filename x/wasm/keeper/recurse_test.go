...
func TestLimitRecursiveQueryGas(t *testing.T) {
	...
	const (
		// Note: about 100 SDK gas (10k CosmWasm gas) for each round of sha256
		GasWork2k uint64 = 76_279 // = NewContractInstanceCosts + x // we have 6x gas used in cpu than in the instance
	...
	cases := map[string]struct {
		...
		"very deep recursion, hits recursion limit": {
			...
			expectedGas:               10*(GasWork2k+GasReturnHashed) + 631, // NOTE: investigation on this is required, higher consumption than vanila wasmd, also needs quantification instead of arbitrary
		},
	}
