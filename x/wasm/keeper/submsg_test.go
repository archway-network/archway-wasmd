...
func TestDispatchSubMsgErrorHandling(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"send tokens": {
			submsgID:         5,
			msg:              validBankSend,
			resultAssertions: []assertion{assertReturnedEvents(0), assertGasUsed(111_000, 112_000)},
		},
		"not enough tokens": {
			submsgID:    6,
			msg:         invalidBankSend,
			...
			resultAssertions: []assertion{assertGasUsed(83_000, 84_000), assertErrorString("codespace: sdk, code: 5")},
		},
		...
		"send tokens with limit": {
			...
			resultAssertions: []assertion{assertReturnedEvents(0), assertGasUsed(112_000, 113_000)},
		},
		"not enough tokens with limit": {
			...
			gasLimit:    &subGasLimit,
			// uses same gas as call without limit (note we do not charge the 40k on reply)
			resultAssertions: []assertion{assertGasUsed(84_000, 85_000), assertErrorString("codespace: sdk, code: 5")},
		},
		"out of gas caught with gas limit": {
			...
			resultAssertions: []assertion{assertGasUsed(subGasLimit+79_000, subGasLimit+80_000), assertErrorString("codespace: sdk, code: 11")},
		},
