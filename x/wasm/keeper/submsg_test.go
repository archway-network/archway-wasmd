...
func TestDispatchSubMsgErrorHandling(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"send tokens": {
			submsgID:         5,
			msg:              validBankSend,
			resultAssertions: []assertion{assertReturnedEvents(0), assertGasUsed(110_000, 124_000)},
		},
		"not enough tokens": {
			submsgID:    6,
			msg:         invalidBankSend,
			...
			resultAssertions: []assertion{assertGasUsed(83000, 90000), assertErrorString("codespace: sdk, code: 5")},
		},
		...
		"send tokens with limit": {
			...
			resultAssertions: []assertion{assertReturnedEvents(0), assertGasUsed(112000, 123000)},
		},
		"not enough tokens with limit": {
			...
			gasLimit:    &subGasLimit,
			// uses same gas as call without limit (note we do not charge the 40k on reply)
			resultAssertions: []assertion{assertGasUsed(84000, 90000), assertErrorString("codespace: sdk, code: 5")},
		},
		"out of gas caught with gas limit": {
			...
			resultAssertions: []assertion{assertGasUsed(subGasLimit+79000, subGasLimit+90000), assertErrorString("codespace: sdk, code: 11")},
		},
...
func TestMigrateGovSubMsgAuthzPropagated(t *testing.T) {
	...
	mockWasmVM.MigrateFn = func(codeID wasmvm.Checksum, env wasmvmtypes.Env, migrateMsg []byte, store types.PrefixStoreInfo, goapi wasmvm.GoAPI, querier types.QuerierWithCtx, gasMeter wasmvm.GasMeter, gasLimit uint64, deserCost wasmvmtypes.UFraction) (*wasmvmtypes.ContractResult, uint64, error) {
		if instanceLevel == 1 {
			return &wasmvmtypes.ContractResult{Ok: &wasmvmtypes.Response{}}, 0, nil
		}
		instanceLevel++
