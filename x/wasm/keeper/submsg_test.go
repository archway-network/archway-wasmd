...
func TestDispatchSubMsgErrorHandling(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"send tokens": {
			submsgID:         5,
			msg:              validBankSend,
			resultAssertions: []assertion{assertReturnedEvents(0), assertGasUsed(120_000, 121_000)},
		},
		"not enough tokens": {
			submsgID:    6,
			msg:         invalidBankSend,
			...
			resultAssertions: []assertion{assertGasUsed(87_000, 88_000), assertErrorString("codespace: sdk, code: 5")},
		},
		...
		"out of gas caught with gas limit": {
			...
			resultAssertions: []assertion{assertGasUsed(subGasLimit+83_000, subGasLimit+84_000), assertErrorString("codespace: sdk, code: 11")},
		},
